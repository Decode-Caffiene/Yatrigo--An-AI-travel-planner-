...
function TripDetailContent({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadTrip = () => {
    ...
    getTrip(token, id)
      .then((res) => {
        ...
        setFlightForm({
          airline: res.trip.flight?.airline || "",
          airlineCode: res.trip.flight?.airlineCode || "",
          flightNumber: res.trip.flight?.flightNumber || "",
          departureAirport: res.trip.flight?.departureAirport || "",
          arrivalAirport: res.trip.flight?.arrivalAirport || "",
          departureTime: toDateTimeInputValue(res.trip.flight?.departureTime),
          arrivalTime: toDateTimeInputValue(res.trip.flight?.arrivalTime),
          confirmationNumber: res.trip.flight?.confirmationNumber || "",
        });
      })
      .catch((err) =>
  ...
  const handleFlightFormSubmit = async () => {
    ...
    try {
      ...
      const result = await updateTrip(token, id, { flight });
      setTrip(result.trip);
  ...
  const hasFlight = !!(trip.flight?.airline || trip.flight?.flightNumber);
            ...
            {!isEditingFlight &&
              (hasFlight ? (
                <div className="flex items-start gap-3">
                  {airlineLogoUrl(trip.flight?.airlineCode) && (
                    <Image
                      src={airlineLogoUrl(trip.flight?.airlineCode)!}
                      alt={trip.flight?.airline || "Airline"}
                      width={40}
                      height={40}
                  ...
                  {(trip.flight?.departureTime || trip.flight?.arrivalTime) && (
                    <p className="font-body-sm text-body-sm text-on-surface-variant">
                      {trip.flight?.departureTime
                        ? new Date(trip.flight.departureTime).toLocaleString()
                        : "—"}{" "}
                      -{" "}
                      {trip.flight?.arrivalTime
                        ? new Date(trip.flight.arrivalTime).toLocaleString()
                        : "—"}
                    </p>
                  )}
                  {trip.flight?.confirmationNumber && (
                    <p className="font-body-sm text-body-sm text-on-surface-variant">
                      Confirmation: {trip.flight.confirmationNumber}
                    </p>
                  )}
                  </div>
                </div>
